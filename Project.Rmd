...
greedy_algorithm <- function(G, b = round(0.1 * vcount(G)), max_walk_length = 10) {
  ...
  top_nodes <- order(node_benefits, decreasing = TRUE)[1:b]
  S <- V(G)[top_nodes]
...
```{r}
# Extended SIS simulation function with vaccination strategy
SISsim_vaccination <- function(G, beta, gamma, vaccination_strategy,  L = 10, S_size = 10, max_steps = 100, num_vaccinated = round(0.1 * vcount(G)), case_type = "case1") {
