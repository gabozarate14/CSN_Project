...
for (network_idx in 1:length(networks)) {
    ...
    for (vaccination_strategy in vaccination_strategies) {
        ...
        for (num_vaccinated in seq(2, 10, by = 2)) {
            for (alpha in alphas) {
               ...
               result_df <- data.frame(res$prop_infected, res$num_infected)
...
for (network_idx in 1:length(networks)) {
    ...
    for (vaccination_strategy in vaccination_strategies) {
        ...
        for (num_vaccinated in c(4,8)) {
             ...
             res <- SISsim_vaccination(G, beta, gamma, vacc_nodes_file, max_steps = 100, 
                                       num_vaccinated = num_vaccinated, case_type = "case2", alpha = alpha, S_size = S_size)
             out_filename = paste("sim/c2_", network_name, "_", vaccination_strategy,"_",num_vaccinated,"_",alpha, ".csv", sep="")
             result_df <- data.frame(res$prop_infected, res$num_infected)
             write.csv(result_df, file = out_filename, row.names = FALSE)
